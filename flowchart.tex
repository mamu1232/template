% Define ブロック styles
\tikzstyle{ブロック} = [rectangle, draw, fill=blue!20, text width=5em,text centered, rounded corners, minimum height=4em]
\tikzstyle{直結} = [draw, -latex']
\tikzstyle{矢印} = [draw, -latex']
\tikzstyle{楕円} = [draw, ellipse,fill=red!20, minimum height=2em]
\tikzstyle{吹出し} = [draw, cloud callout,fill=red!20,]
%%%%%
\tikzstyle{未定義} = [cross out,draw=red]

\tikzstyle{処理} = [rectangle, draw, fill=blue!20, text width=5em,text centered, rounded corners, minimum height=4em]
\tikzstyle{入出力} = [trapezium, draw, fill=blue!20, trapezium left angle=60, trapezium right angle=120, minimum height=4em,trapezium stretches]
\tikzstyle{判断} = [diamond, draw, fill=blue!20,text width=4.5em,text badly centered, inner sep=0pt]
\tikzstyle{接続子} = [circle, draw, fill=blue!20]
\tikzstyle{表示} = [未定義]

\tikzstyle{処理ファイル} = [rounded rectangle, draw, fill=blue!20, rounded rectangle arc length=90, rounded rectangle east arc=concave, minimum height=4em]
\tikzstyle{コネクタ} = [chamfered rectangle, draw, fill=blue!20, chamfered rectangle corners={south west, south east}, chamfered rectangle ysep=1em, minimum height=2em ]
\tikzstyle{文書} = [tape, draw, fill=blue!20, tape bend top=none, minimum size=4em]
\tikzstyle{手動操作} = [trapezium, draw, fill=blue!20, shape border rotate=180, minimum height=4em,trapezium stretches]
\tikzstyle{準備} = [chamfered rectangle, draw, fill=blue!20, chamfered rectangle xsep=4em, minimum width=6em]

\tikzstyle{手動入力} = [trapezium, draw, fill=blue!20, trapezium left angle=90, trapezium right angle=75, shape border rotate=90]
\tikzstyle{定義済み処理} = [draw, fill=blue!20,rectangle split, rectangle split horizontal,rectangle split parts=3,minimum height=1cm]
\tikzstyle{端子} = [rounded rectangle, draw, fill=blue!20, minimum width=4em]

\tikzstyle{ループ始} = [chamfered rectangle, draw, fill=blue!20, chamfered rectangle corners={north west, north east}, chamfered rectangle xsep=2em, minimum height=4em]
\tikzstyle{ループ終} = [chamfered rectangle, draw, fill=blue!20, chamfered rectangle corners={south west, south east}, chamfered rectangle xsep=2em, minimum height=4em ]
\tikzstyle{吹出し角} = [draw, rectangle callout,fill=red!20,]
\tikzstyle{吹出し丸} = [draw, ellipse callout,fill=red!20,]
\tikzstyle{吹出し雲} = [draw, cloud callout,fill=red!20,]
